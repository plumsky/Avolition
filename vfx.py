...
class short_vfx():
    def __init__(self, parent, texture=None, scale=1.0, Z=0.0, depthTest=False, depthWrite=False):
        ...
        if texture != None:
            self.vfx.setTexture(TextureStage.getDefault(),
                                loader.loadTexture(texture), 1)
...
class MovingVfx():
    ...
    def start(self):
        speed = self.time/64
        taskMgr.doMethodLater(speed, self.run, 'vfx')
        self.interval = ProjectileInterval(self.vfx, startPos=self.from_target.getPos(
            render), endPos=self.target.getPos(render), duration=self.time, gravityMult=self.gravity).start()
