...
class SoundPool():
    def __init__(self, common):
        ...
        for node in range(11):
            self.sound_nodes.append(
                render.attachNewNode("soundNode"+str(node)))
            ...
            for sound in self.sounds[-1]:
                self.audio3d.attachSoundToObject(
                    self.sounds[-1][sound], self.sound_nodes[-1])
        self.targets = {}
    ...
    def update(self, task):
        for target in self.targets:
            if self.targets[target]:
                self.sound_nodes[target].setPos(
                    self.targets[target].getPos(render))
        return task.again
