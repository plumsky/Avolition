...
class VisibilityPolygon():
    def compute(self, position, segments):
        ...
        while i < (len(sorted)):
            ...
            while (sorted[i][2] < sorted[orig][2] + self.epsilon()):
                if (map[sorted[i][0]] != -1):
                    if (sorted[i][0] == old_segment):
                        extend = True
                        vertex = segments[sorted[i][0]][sorted[i][1]]
                    self.remove(map[sorted[i][0]], heap,
                                position, segments, vertex, map)
                else:
                    self.insert(sorted[i][0], heap, position,
                                segments, vertex, map)
            ...
            if extend:
                ...
                cur = self.intersectLines(
                    segments[heap[0]][0], segments[heap[0]][1], position, vertex)
                if not self.equal(cur, vertex):
                    polygon.append(cur)
                    # print "extend"
            elif shorten:
                # print "shorten"
                polygon.append(self.intersectLines(
                    segments[old_segment][0], segments[old_segment][1], position, vertex))
                polygon.append(self.intersectLines(
                    segments[heap[0]][0], segments[heap[0]][1], position, vertex))
                # print "shorten"
        return polygon
    ...
    def sortPoints(self, position, segments):
        ...
        for i in xrange(len(segments)):
            for j in range(2):
                ...
                points[2*i+j] = [i, j, a]
        # is this the same?? points.sort(function(a,b) {return a[2]-b[2];});
    ...
    def insert(self, index, heap, position, segments, destination, map):
        print("insert")
        intersect = self.intersectLines(
            segments[index][0], segments[index][1], position, destination)
        if len(intersect) == 0:
            return
        cur = len(heap)
        heap.append(index)
    ...
    def intersectLines(self, a1, a2, b1, b2):
        ...
        b1 = map(float, b1)
        b2 = map(float, b2)
        ua_t = (b2[0] - b1[0]) * (a1[1] - b1[1]) - \
            (b2[1] - b1[1]) * (a1[0] - b1[0])
        ub_t = (a2[0] - a1[0]) * (a1[1] - b1[1]) - \
            (a2[1] - a1[1]) * (a1[0] - b1[0])
        u_b = (b2[1] - b1[1]) * (a2[0] - a1[0]) - \
            (b2[0] - b1[0]) * (a2[1] - a1[1])
        if (u_b != 0):
            ua = ua_t / u_b
            ub = ub_t / u_b
    ...
    def lessThan(self, index1, index2, position, segments, destination):
        # print "lessThan"
        inter1 = self.intersectLines(
            segments[index1][0], segments[index1][1], position, destination)
        inter2 = self.intersectLines(
            segments[index2][0], segments[index2][1], position, destination)
...
if __name__ == '__main__':
    ...
    polygons.append([[-24.0, -5.0], [-25.5, -5.0],
                     [-25.5, -4.0], [-24.0, -4.0]])
    polygons.append([[-24.0, 5.0], [-24.0, 4.0], [-25.5, 4.0], [-25.5, 5.0]])
    polygons.append([[-34.0, 4.0], [-43.0, 4.0], [-42.5, -4.0], [-34.0, -4.0], [-32.0, -12.0], [-29.0, -14.0], [-24.0, -14.0], [-24.0, -23.5], [-16.0, -23.5],
                     [-16.0, -4.0], [4.0, -4.0], [4.0, 4.0], [-16.0, 4.0], [-16.0, 14.0], [-26.0, 14.0], [-26.0, 34.0], [-44.0, 34.0], [-44.0, 26.5], [-34.0, 26.0]])
    ...
    draw.ellipse((position[0]-3, position[1]-3,
                  position[0]+3, position[1]+3), fill='blue')
    del draw
