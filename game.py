...
class Game(DirectObject):
    def __init__(self, loadscreen, root):
        ...
        # audio 3d
        self.common['audio3d'] = Audio3DManager.Audio3DManager(
        ...
        if not config_safemode.getValue():
            if config_bloom.getValue():
                self.bloomBuffer = base.win.makeTextureBuffer(
                    "bloom", buff_size.getValue(), buff_size.getValue())
            ...
            self.shadowBuffer = base.win.makeTextureBuffer(
                "Shadow Buffer", buff_size.getValue(), buff_size.getValue(), self.shadowTexture)
