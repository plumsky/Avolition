...
class Interactive():
    ...
    def __init__(self, common, proto, pos, model=None, scale=None, gui=None, command=None):
        ...
        ProjectileInterval(self.model, startPos=(
            pos[0], pos[1], pos[2]+1), endPos=pos, duration=1.0, gravityMult=1.0).start()
        # print "hop!"
...
class Monster():
    def __init__(self, setup_data, common, level=1.0, start_pos=(0, 0, 0)):
        ...
        self.rootBone = self.actor.exposeJoint(
            None, 'modelRoot', setup_data["root_bone"])
        ...
        self.PC = common['PC']
        self.speed_mode = random.randrange(
            0+int(level), 42+int(level), 7)/100.0
    ...
    def damageOverTime(self, task):
        ...
        if self.DOT > 0:
            ...
            if self.stats['hp'] < 1:
                ...
                self.common['soundPool'].play(
                    self.soundID, self.sound_names["die"])
                self.state = "DIE"
            vfx(self.node, texture=self.vfx, scale=.5, Z=1.0,
                depthTest=True, depthWrite=True).start(0.016, 24)
        return task.again
    ...
    def onMagmaHit(self):
        ...
        if self.stats['hp'] < 1:
            ...
            self.state = "DIE"
            vfx(self.node, texture=self.vfx, scale=.5, Z=1.0,
                depthTest=True, depthWrite=True).start(0.016)
    ...
    def onSparkHit(self, damage):
        ...
        if self.stats['hp'] < 1:
            ...
            short_vfx(self.node, texture="vfx/short_spark.png", scale=.5,
                      Z=1.0, depthTest=True, depthWrite=True).start(0.03)
    ...
    def onHit(self, damage, sound="hit"):
        ...
        if self.stats['hp'] < 1:
            ...
            if sound:
                self.common['soundPool'].play(
                    self.soundID, self.sound_names[sound])
            self.common['soundPool'].play(
                self.soundID, self.sound_names["die"])
            self.state = "DIE"
        else:
    ...
    def runAI(self, task):
        ...
        if self.state == "DIE":
            ...
            if self.common["kills"] == 0:
                Interactive(
                    self.common, data.items['key'], self.node.getPos(render))
            elif random.randrange(10) == 0:
                Interactive(
                    self.common, data.items['potion'], self.node.getPos(render))
            Sequence(Wait(2.0), LerpPosInterval(self.node, 2.0, VBase3(self.node.getX(
            ), self.node.getY(), self.node.getZ()-5)), Func(self.destroy)).start()
            return task.done
            ...
            if(self.actor.getCurrentAnim() != "attack"):
                ...
                Sequence(Wait(
                    self.attack_pattern[-1]), Func(self.attack, list(self.attack_pattern))).start()
            return task.again
...
class Spawner():
    ...
    def update(self, task):
        ...
        if points_in_range:
            final_point = points_in_range.pop(
                random.randrange(len(points_in_range)-1))
            ...
            Monster(data.monsters[random.choice(self.monster_type)].copy(
            ), self.common, self.level, final_point.getPos(render))
            # self.level+=0.1
            # print "spawn!"
        return task.again
...
class MusicPlayer():
    ...
    def playAll(self, skip=1):
        ...
        self.seq = Sequence(LerpFunc(base.musicManager.setVolume, fromData=self.volume, toData=0.0, duration=1.0),
                            ...
                            LerpFunc(base.musicManager.setVolume,
                                     fromData=0.0, toData=self.volume, duration=1.0),
                            Wait(time-2.0),
                            Func(self.playAll))
        self.seq.start()
    ...
    def loop(self, track=0, fadeIn=False):
        if fadeIn:
            ...
            LerpFunc(base.musicManager.setVolume, fromData=0.0,
                     toData=self.volume, duration=5.0).start()
...
class LevelLoader():
    ...
    def load(self, level=0, PCLoad=True):
        ...
        if 'PC' in self.common and PCLoad:
            ...
            pos = (data.levels[level]["enter"][0], data.levels[level]
                   ["enter"][1], data.levels[level]["enter"][2])
            self.common['PC'].node.setPos(pos)

        pos = (data.levels[level]["exit"][0], data.levels[level]
               ["exit"][1], data.levels[level]["exit"][2])
        ...
        while True:
            ...
            if not node.isEmpty():
                ...
                self.common['waypoints_data'].append(json.loads(node.getTag("path"))[
                                                     ::-1])  # data is in reverse order... but why?
                i += 1
            else:
                break
        ...
        # spawner
        self.common['spawner'].start(
            data.levels[level]["map_monsters"], 1.0, data.levels[level]["num_monsters"])
