...
class CharGen(DirectObject):
    ...
    def load(self):
        ...
        self.fire = vfx(self.fire_node, texture='vfx/big_fire3.png',
                        scale=.29, Z=.5, depthTest=True, depthWrite=True)
        self.fire.show()
        self.fire.loop(0.02)

        self.character1 = Actor("models/pc/male", {"attack": "models/pc/male_attack2",
                                                   "idle": "models/pc/male_ready2", "block": "models/pc/male_block"})
        ...
        if self.common['nude']:
            self.character2 = Actor(
                "models/pc/female_nude", {"attack": "models/pc/female_attack1", "idle": "models/pc/female_idle"})
        else:
            self.character2 = Actor(
                "models/pc/female", {"attack": "models/pc/female_attack1", "idle": "models/pc/female_idle"})
        ...
        if self.common['nude']:
            self.character3 = Actor("models/pc/female2_nude", {
                                    "attack": "models/pc/female2_arm", "reset": "models/pc/female2_fire", "idle": "models/pc/female2_idle"})
        else:
            self.character3 = Actor("models/pc/female2", {"attack": "models/pc/female2_arm",
                                                          "reset": "models/pc/female2_fire", "idle": "models/pc/female2_idle"})
        #self.character2.setPlayRate(.4, "attack")
        ...
        self.arrow_bone = self.character3.exposeJoint(
        ...
        self.character4 = Actor(
            "models/pc/male2", {"attack": "models/pc/male2_aura", "idle": "models/pc/male2_idle"})
        #self.character2.setPlayRate(.4, "attack")
        ...
        self.MousePickerNode.addSolid(self.pickerRay)
        self.common['traverser'].addCollider(
            self.pickerNP, self.common['queue'])
        ...
        self.camLoop = Sequence(LerpHprInterval(self.cameraNode, 10.0, VBase3(-20, 0, 0), bakeInStart=0),
                                LerpHprInterval(self.cameraNode, 10.0, VBase3(20, 0, 0), bakeInStart=0))
        self.camLoop.loop()
    ...
    def onStart(self, event=None):
        ...
        if self.current_class == "1":
            ...
            #self.common['PC'].node.setPos(-12, 0, 0)
        pos = (data.levels[self.currentLevel]["enter"][0], data.levels[self.currentLevel]
               ["enter"][1], data.levels[self.currentLevel]["enter"][2])
    ...
    def GUIOnEnter(self, object, event=None):
        ...
        self.Tooltip['text'] = self.tooltip_text[int(
            self.current_class)][object]+self.getSliderValue(object)
        if object[1] == "A":
    ...
    def onClick(self):
        ...
        if my_class == "1":
            ...
            self.slider2.show()
            self.slider3.show()
            Sequence(self.character1.actorInterval("attack"), self.character1.actorInterval(
                "block"), Func(self.loopAnim, self.character1, "idle")).start()
            ...
            self.slider2.show()
            self.slider3.show()
            Sequence(self.character2.actorInterval("attack", playRate=0.8), Func(
                self.loopAnim, self.character2, "idle")).start()
            ...
            Sequence(Wait(1.5), Func(self.fireArrow), Func(self.character3.play, "reset"), Wait(
                1.0), Func(self.loopAnim, self.character3, "idle")).start()
        elif my_class == "4":
            self.slider1['value'] = 50
            self.slider2['value'] = 50
            self.slider3['value'] = 50
            ...
            aura = vfx(self.character4, texture='vfx/tele2.png',
                       scale=.5, Z=.85, depthTest=False, depthWrite=False)
            aura.show()
            aura.start()
            self.FFSound.play()
            Sequence(Wait(2.2), Func(self.loopAnim,
                                     self.character4, "idle")).start()
